...
func (r *FuelEntryRepository) GetAll(ctx context.Context) ([]*fuelentry.FuelEntry, error) {
	rows, err := r.db.Query(ctx, `
		SELECT id, liters, total_cost, price_per_l, odometer, fuelled_at, created_at, updated_at, deleted_at
...
func (r *FuelEntryRepository) Save(ctx context.Context, e *fuelentry.FuelEntry) error {
	_, err := r.db.Exec(ctx, `
		INSERT INTO fuel_entries (id, liters, total_cost, price_per_l, odometer, fuelled_at, created_at, updated_at, deleted_at)
		VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`,
		e.ID, e.Liters, e.TotalCost, e.PricePerL, e.Odometer, e.FuelledAt, e.CreatedAt, e.UpdatedAt, e.DeletedAt)
	return err
}

func (r *FuelEntryRepository) Update(ctx context.Context, e *fuelentry.FuelEntry) error {
	_, err := r.db.Exec(ctx, `
		UPDATE fuel_entries SET liters=$2, total_cost=$3, price_per_l=$4, odometer=$5,
		fuelled_at=$6, updated_at=$7, deleted_at=$8 WHERE id=$1`,
		e.ID, e.Liters, e.TotalCost, e.PricePerL, e.Odometer, e.FuelledAt, e.UpdatedAt, e.DeletedAt)
	return err
}
